...
async fn openai_transcribe(audio: Vec<u8>, mime_type: String) -> Result<String, String> {
    ...
    let filename = if mime_type.contains("mp4") { "speech.mp4" } else { "speech.webm" };
    ...
    let response = reqwest::Client::new()
        ...
        .multipart(form)
        .send().await
        .map_err(|_| "The transcription service could not be reached. Your recording remains on this device.".to_string())?;
    let status = response.status();
...
async fn openai_respond(input: String, conversation: Vec<Value>) -> Result<String, String> {
    ...
    let status = response.status();
    let value: Value = response.json().await
        .map_err(|_| "The AI service returned an unreadable response. Nothing was changed.".to_string())?;
    if !status.is_success() {
        return Err(openai_safe_error(status.as_u16(), &value));
    }
    extract_output_text(&value).ok_or_else(|| "The AI response did not contain readable text.".to_string())
}
...
async fn openai_speech(text: String) -> Result<Vec<u8>, String> {
    ...
    let response = reqwest::Client::new()
        ...
        .json(&body)
        .send().await
        .map_err(|_| "The speaking service could not be reached. The written answer is still available.".to_string())?;
    let status = response.status();
    if !status.is_success() {
        return Err(format!("The speaking service could not complete the request ({}).", status.as_u16()));
    }
...
fn extract_output_text(value: &Value) -> Option<String> {
    ...
    value.get("output")?.as_array()?.iter()
        ...
        .find_map(|content| content.get("text").and_then(Value::as_str).map(str::to_string))
}
...
fn openai_safe_error(status: u16, value: &Value) -> String {
    ...
    match (status, code) {
        ...
        (429, "insufficient_quota") => "The OpenAI account does not currently have available API credit.".to_string(),
        (429, _) => "The AI service is busy or its usage limit was reached. Please try again shortly.".to_string(),
        _ => format!("The AI service could not complete the request ({}). Nothing was changed.", status)
    }
}
...
mod tests {
    ...
    fn extracts_responses_output_text() {
        let response = serde_json::json!({"output":[{"content":[{"type":"output_text","text":"Answer"}]}]});
